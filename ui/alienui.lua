...
function perkFrame()
	...
	if (not showUI) then do return false end end

	local current_level = GetPlayerLevel();
	local current_xp = math.min(GetPlayerXP(), GetLevelUpCost());

	GuiLayoutBeginVertical( ALIEN_gui, 29, 18 )
	GuiText(ALIEN_gui, 0, 0, "XP : "..tostring(current_xp).." / " .. tostring(GetLevelUpCost()))
	GuiText(ALIEN_gui, 0, 0, "Level : "..tostring(current_level))
	...
	if (GetAvailablePerkNum() ~= 0) then
		if (#perkDataList == 0) then perkDataList = GetAvailablePerks() end
	...
	if (#StoredPerkSetIndices ~= 0) then
		...
		for _, i in pairs(StoredPerkSetIndices) do
			biomeName = GetStoredBiome(i)
			if (biomeName == nil) then break end
			if (i >= 4) then
				x_offset = 100
				y_offset = -30
			end
			if GuiButton( ALIEN_gui, 185 + x_offset, 114+10*i + y_offset, "Perk Set "..i.." ("..biomeName..")", storeBtnPos+i) then
				LoadStoredPerkSet(i)
				refreshPerkList = true
			end
		end
